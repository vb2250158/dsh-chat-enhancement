...
在同一聊天流或 Markdown 预览内选择文字，点击选区附近的「批注」。面板保存选中文字，编辑批注时不依赖浏览器仍保留选区。「添加到聊天」追加引用原文、可用的文件名或消息标识以及批注；可重复添加多个引用，不自动发送。Enter 换行，Ctrl/Cmd+Enter 添加，Escape、「取消」或点击面板外部关闭并丢弃未添加批注；输入法确认不会触发添加。空白批注不能添加。编辑器、输入框内的选文以及跨聊天流选文不会触发。
