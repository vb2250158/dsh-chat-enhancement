...
选区批注通过「添加为附件」进入附件栏，保持草稿正文不变，也不自动发送。已支持新版 DSH 的 `addAttachmentFiles`；在尚未提供该方法的现有安装版本上，复用文件选择器的草稿附件入口。添加失败时保留批注内容供重试。已在原安装版本的真实页面验证 JSON 附件上传，并移除验收时的临时附件。
...
- **选区批注改走附件**：批注不再拼成草稿文本，而是作为一份 `批注-<序号或时间>.json` 文件进入附件栏。`ctx.conversation.addAttachmentFiles(sessionId, files)` 与用户选文件使用同一草稿接收路径，草稿文本、引用芯片和其它附件都不受影响，也不会自动发送。载荷是带 `schema` 标记的 JSON，记录引用原文、批注内容与目标坐标（`msgKey` / `msgKind` / `seq`）。
...
在同一聊天流或 Markdown 预览内选择文字，点击选区附近的「批注」。面板保存选中文字，编辑批注时不依赖浏览器仍保留选区。「添加为附件」把这条批注作为一份 `批注-<序号或时间>.json` 文件放入输入区的附件栏，不自动发送，也不把引用与意见拼成草稿文本。可重复添加多条，草稿里已有的文本、引用芯片和其它附件都不受影响。Enter 换行，Ctrl/Cmd+Enter 添加，Escape、「取消」或点击面板外部关闭并丢弃未添加批注；输入法确认不会触发添加。空白批注不能添加。编辑器、输入框内的选文以及跨聊天流选文不会触发。

批注面板是当前会话的临时状态，切换会话或刷新会丢弃尚未添加的批注。添加失败（会话已切换、输入区处于命令或提交中等）时保留文字并显示重试提示。附件优先走 `ctx.conversation.addAttachmentFiles()`；旧版 DSH 缺少该方法时，复用其文件选择器的草稿创建、附件栏和释放入口。因此上传、预览与提交行为一致。载荷是带 `schema` 标记的 JSON，记录引用原文、批注内容与目标坐标（`msgKey` / `msgKind` / `seq`），模型侧可直接读该文件而无需用户再粘贴上下文。除这份附件外不建立第二份批注存储。
