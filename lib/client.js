  ...
  factory: (require) => {
    ...
    function ToolCallGroupController() {
      ...
      React.useEffect(() => {
        ...
        const sync = () => {
          const previous = groupsRef.current; const next = new Map()
          for (const flow of document.querySelectorAll('[data-chat-flow]')) {
            ...
            for (const child of children) { if (isGroupedActivity(child.dataset.chatFlowKind)) run.push(child); else { flush(); run = [] } }
            flush()
          }
          for (const [key, group] of previous) { if (next.has(key)) continue; group.button.remove(); for (const row of group.rows) row.hidden = false; expandedRef.current.delete(key) }
