        ...
        const sync = () => {
          ...
          for (const flow of document.querySelectorAll('[data-chat-flow]')) {
            ...
            const flush = () => {
              ...
              Object.assign(group.button.style, toolGroupButtonStyle); group.button.setAttribute('aria-expanded', String(expanded)); group.button.textContent = `${expanded ? '⌄' : '›'} 工具调用 · ${run.length} 项`
            ...
            for (const child of children) { if (child.dataset.chatFlowKind === 'tool-call') run.push(child); else { flush(); run = [] } }
            flush()
          }
