window.__ModuleLoader__.load({
  ...
  factory: (require) => {
          ...
          const sync = () => {
            ...
            for (const parent of parentNodes()) {
              ...
              const flush = () => {
                ...
                if (!expanded) inlineTrailingActivity(trailingRow, group.button, nextInlineRows); else { group.button.style.flex = ''; group.button.style.width = '100%'; if (group.button.parentElement !== parent || group.button.nextElementSibling !== hiddenRows[0]) parent.insertBefore(group.button, hiddenRows[0]) }
    ...
    const ToolCallActivityGroupController = createActivityGroupController({ toggleKey: 'dshChatEnhancementToolGroupToggle', parentNodes: () => document.querySelectorAll('[data-chat-flow]'), isActivity: (row) => isGroupedActivity(row.dataset.chatFlowKind) && !isDisplayToolRow(row), groupKey: toolGroupKey, label: (count) => `已执行 ${count} 项操作` })
    let nextReasoningKey = 0
    function reasoningGroupKey(rows) { return rows.map((row) => { if (row.dataset.dshChatEnhancementReasoningKey === undefined) row.dataset.dshChatEnhancementReasoningKey = `reasoning-${nextReasoningKey++}`; return row.dataset.dshChatEnhancementReasoningKey }).join('|') }
    const ThinkingActivityGroupController = createActivityGroupController({ toggleKey: 'dshChatEnhancementThinkingGroupToggle', parentNodes: () => new Set([...document.querySelectorAll('[data-variant="think"]')].map((row) => row.parentElement).filter((parent) => parent !== null)), isActivity: (row) => row.dataset.variant === 'think', groupKey: reasoningGroupKey, label: (count) => `已完成 ${count} 项思考` })
