...
/** Selection annotations enter the composer as generated file attachments. */
...
const annotationLocales = {
  zh: {
    ...
    mark: '批注', quote: '引用原文', note: '批注内容', add: '添加为附件', cancel: '取消',
  ...
  en: {
    ...
    mark: 'Annotate', quote: 'Quoted text', note: 'Comment', add: 'Attach comment', cancel: 'Cancel',
 ...
 * the ordinary attachment path. `target.seq` is the
 ...
 * spends chat context as prose. The draft's existing text,
...
function appendAnnotation(sessions, conversation, sessionId, annotation) {
  ...
  if (input.state.getSnapshot().phase !== 'plain') return 'unavailable'
  ...
  try {
    if (typeof conversation.addAttachmentFiles === 'function') {
      return conversation.addAttachmentFiles(sessionId, [file]).length === 1 ? null : 'unavailable'
    }
  } catch {
    return 'unavailable'
  }
  // Older DSH builds expose the same picker primitives before the generated
  // file intake method. Keep the annotation inside their ordinary draft rail.
  if (typeof conversation.createDrafts !== 'function' || typeof conversation.releaseDraftAttachments !== 'function') return 'unavailable'
  let drafts
  try {
    drafts = conversation.createDrafts(sessionId, [file])
    const ids = drafts.map(draft => draft.id)
    if (ids.length === 1 && input.addAttachments(ids)) return null
  } catch {
    // The draft or input can become unavailable while the file is registered.
  }
  if (drafts !== undefined) conversation.releaseDraftAttachments(drafts)
  return 'unavailable'
}
