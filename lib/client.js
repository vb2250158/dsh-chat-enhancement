window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function createMediaAutoplayGate() {
      const observedCallIds = new Set(); let openedAt = Number.POSITIVE_INFINITY
      return { markReady(time = Date.now()) { openedAt = time }, observe(callId, resultTime, enabled, running) { if (observedCallIds.has(callId)) return false; observedCallIds.add(callId); return resultTime >= openedAt && enabled && running } }
    }
    const mediaAutoplayGates = new Map()
    function mediaAutoplayGate(sessionId) { let gate = mediaAutoplayGates.get(sessionId); if (gate === undefined) { gate = createMediaAutoplayGate(); mediaAutoplayGates.set(sessionId, gate) }; return gate }
    function MediaAutoplaySessionController({ sessionId }) {
      const gate = mediaAutoplayGate(sessionId)
      React.useEffect(() => { let mounted = true; queueMicrotask(() => { if (mounted) gate.markReady() }); return () => { mounted = false; mediaAutoplayGates.delete(sessionId) } }, [gate, sessionId])
      return null
    }
    ...
    function ImagePreviewDialog({ sessionId, initial, onClose }) {
      ...
      return React.createElement('div', {
        ...
        onPointerUp: (event) => { if (pointerStart.current === null) return; const distance = event.clientX - pointerStart.current; pointerStart.current = null; if (Math.abs(distance) > 50) swiped.current = true; if (distance > 50) navigate(-1); else if (distance < -50) navigate(1) },
      },
      ...
      controlsVisible && React.createElement('div', { style: imageDialogToolbarStyle, onClick: (event) => event.stopPropagation() }, React.createElement('a', { href: current.url, download: current.name, style: imageDialogActionStyle }, '下载'), React.createElement('button', { type: 'button', style: imageDialogActionStyle, onClick: onClose, 'aria-label': '关闭图片预览' }, '关闭')),
      ...
      React.createElement('img', { src: current.url, alt: current.name, draggable: false, role: 'button', tabIndex: 0, 'aria-label': controlsVisible ? '隐藏图片预览控件' : '显示图片预览控件', onClick: (event) => { event.stopPropagation(); if (swiped.current) { swiped.current = false; return }; setControlsVisible((value) => !value) }, onKeyDown: (event) => { if (event.key !== 'Enter' && event.key !== ' ') return; event.preventDefault(); setControlsVisible((value) => !value) }, style: floating ? { display: 'block', width: '100%', height: 'calc(100% - 18px)', minWidth: 0, minHeight: 0, objectFit: 'contain', userSelect: 'none' } : { display: 'block', maxWidth: 'calc(100vw - 32px)', maxHeight: 'calc(100vh - 32px)', objectFit: 'contain', userSelect: 'none' } }),
    ...
    function VideoPreview({ sessionId, preview, readMedia, shouldAutoplay }) {
      ...
      React.useEffect(() => { if (shouldAutoplay && url !== null) void player.current?.play().catch(() => {}) }, [shouldAutoplay, url])
    ...
    function MediaSettingsSection({ mediaSettings }) {
      ...
      return React.createElement('section', { style: settingsSectionStyle }, React.createElement('h2', null, '媒体播放'), row('audioAutoplay', 'Agent 展示音频时自动播放'), row('videoAutoplay', 'Agent 展示视频时自动播放'), React.createElement('p', { style: mutedStyle }, '只在当前聊天已打开后，Agent 新完成展示调用时尝试播放；进入聊天或恢复历史消息不会播放。浏览器仍可能阻止未交互页面的有声自动播放。'))
    }
    ...
    async function apply(ctx) {
      ...
      ctx.slots.inject('conversation.input.dock', () => ctx.slots.register({ name: 'conversation.input.dock', id: 'chat-enhancement-media-autoplay-session', order: 99 }, MediaAutoplaySessionController))
      ...
      for (const key of ['show_image', 'show_video', 'show_audio']) ctx.slots.inject('tool.call.toolview', () => ctx.slots.register({ name: 'tool.call.toolview', key, locale: 'conversation' }, (props) => React.createElement(MediaToolView, { ...props, sessions, readMedia, mediaSettings })))
      return dispose
    }
    exports.inject = inject
    exports.apply = apply
    exports.createMediaAutoplayGate = createMediaAutoplayGate
