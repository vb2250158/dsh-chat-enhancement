window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function toolNameFromBlock(block) { return 'kind' in block ? block.call?.name ?? '工具调用' : block.name ?? '工具调用' }
    function toolArgsFromBlock(block) { return block.argsRaw ?? block.call?.argsRaw ?? '' }
    function toolOutputFromBlock(block) { return !('kind' in block) ? '' : block.content.filter((part) => part.type === 'text').map((part) => part.text).join('\n') }
    function GenericToolView({ toolName, block }) {
      const args = toolArgsFromBlock(block); const output = toolOutputFromBlock(block)
      return React.createElement('section', { style: genericToolStyle }, React.createElement('strong', null, toolName), args !== '' && React.createElement('pre', { style: toolPayloadStyle }, args), output !== '' && React.createElement('pre', { style: toolPayloadStyle }, output))
    }
    function CollapsibleToolCall({ renderSlot, block, selectedCallId, cwd, openFile, inspectCall }) {
      ...
      const owner = { callId: block.callId, toolName, block, cwd, openFile, inspect: () => { inspectCall(block.callId) } }
      ...
      return React.createElement('section', { 'data-dsh-chat-enhancement': 'tool-group', style: toolGroupStyle },
        React.createElement('button', { type: 'button', style: toolGroupButtonStyle, onClick: () => setExpanded((value) => !value), 'aria-expanded': expanded }, React.createElement('span', { 'aria-hidden': true }, expanded ? '⌄' : '›'), React.createElement('span', null, `工具调用 · ${toolName}`), React.createElement('span', { style: { ...mutedStyle, marginLeft: 'auto' } }, state)),
        expanded && React.createElement('div', { style: toolGroupBodyStyle }, renderSlot('tool.call.toolview', owner, { entryKey: toolName, fallback: React.createElement(GenericToolView, { toolName, block }) }), subCalls.map((child) => React.createElement(CollapsibleToolCall, { key: child.callId, renderSlot, block: child, selectedCallId, cwd, openFile, inspectCall }))),
      )
    }
    function CollapsibleToolCallTree({ renderSlot, node, selectedCallId, cwd, openFile, inspectCall }) { return React.createElement(CollapsibleToolCall, { renderSlot, block: node.data.root, selectedCallId, cwd, openFile, inspectCall }) }
    ...
    async function apply(ctx) {
      ...
      ctx.slots.inject('conversation.chat.node', () => ctx.slots.register({ name: 'conversation.chat.node', key: 'tool-call', priority: -1, locale: 'conversation', children: { 'tool.call.toolview': { kind: 'keyed', scope: 'session' } } }, CollapsibleToolCallTree))
