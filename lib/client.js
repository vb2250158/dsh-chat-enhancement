  ...
  factory: (require) => {
    ...
    function ToolCallGroupController() {
      ...
      React.useEffect(() => {
        ...
        const sync = () => {
          const previous = groupsRef.current; const next = new Map(); const nextRows = new Set()
          for (const flow of document.querySelectorAll('[data-chat-flow]')) {
            ...
            for (const child of children) { if (isGroupedActivity(child.dataset.chatFlowKind) && !isRunningActivity(child)) run.push(child); else { flush(); run = [] } }
            flush()
          }
        ...
        return () => { observer.disconnect(); if (frame !== null) cancelAnimationFrame(frame); for (const group of groupsRef.current.values()) { group.button.remove(); for (const row of group.rows) row.hidden = false }; groupsRef.current.clear(); expandedRef.current.clear() }
      }, [])
      return null
    }
