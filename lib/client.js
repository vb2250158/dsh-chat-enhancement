window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function isRunningActivity(row) { return row.querySelector('[data-state="running"]') !== null }
    function inlineRunningActivity(row, button, inlineRows) { const content = [...row.children].find((child) => child !== button) ?? null; Object.assign(row.style, { alignItems: 'center', display: 'flex', gap: '8px' }); if (content !== null) Object.assign(content.style, { flex: '1 1 auto', minWidth: '0' }); button.style.flex = '0 0 auto'; button.style.width = 'auto'; row.insertBefore(button, content); inlineRows.set(row, content) }
    function clearInlineRunningActivity(row, content) { row.style.alignItems = ''; row.style.display = ''; row.style.gap = ''; if (content !== null) { content.style.flex = ''; content.style.minWidth = '' } }
        ...
        const sync = () => {
          ...
          for (const flow of document.querySelectorAll('[data-chat-flow]')) {
            ...
            const flush = (runningRow = null) => {
              ...
              const key = toolGroupKey(run); const expanded = expandedRef.current.has(key)
              const group = previous.get(key) ?? { button: document.createElement('button'), rows: [] }
              group.rows = run; group.button.type = 'button'; group.button.dataset.dshChatEnhancementToolGroupToggle = key
              Object.assign(group.button.style, toolGroupButtonStyle); group.button.setAttribute('aria-expanded', String(expanded)); group.button.textContent = `${expanded ? '⌄' : '›'} 已执行 ${run.length} 项操作`
              group.button.onclick = () => { if (expandedRef.current.has(key)) expandedRef.current.delete(key); else expandedRef.current.add(key); sync() }
              if (runningRow !== null && !expanded) inlineRunningActivity(runningRow, group.button, nextInlineRows); else { group.button.style.flex = ''; group.button.style.width = '100%'; if (group.button.parentElement !== flow || group.button.nextElementSibling !== run[0]) flow.insertBefore(group.button, run[0]) }
              for (const row of run) row.hidden = !expanded
              for (const row of run) nextRows.add(row)
              next.set(key, group)
            }
            for (const child of children) { if (isGroupedActivity(child.dataset.chatFlowKind) && !isRunningActivity(child)) run.push(child); else { flush(isGroupedActivity(child.dataset.chatFlowKind) && isRunningActivity(child) ? child : null); run = [] } }
            flush()
          }
          ...
          for (const [row, content] of inlineRowsRef.current) { if (!nextInlineRows.has(row)) clearInlineRunningActivity(row, content) }
          groupsRef.current = next
          inlineRowsRef.current = nextInlineRows
        }
        const observer = new MutationObserver((records) => { if (records.some((record) => !(record.target instanceof Element && record.target.closest('[data-dsh-chat-enhancement-tool-group-toggle]') !== null))) schedule() })
        observer.observe(document.body, { attributes: true, attributeFilter: ['data-state'], childList: true, subtree: true }); schedule()
        return () => { observer.disconnect(); if (frame !== null) cancelAnimationFrame(frame); for (const group of groupsRef.current.values()) { group.button.remove(); for (const row of group.rows) row.hidden = false }; for (const [row, content] of inlineRowsRef.current) clearInlineRunningActivity(row, content); groupsRef.current.clear(); expandedRef.current.clear(); inlineRowsRef.current.clear() }
      }, [])
      return null
    }
    ...
    async function apply(ctx) {
      ...
      const readVideo = async (request) => { const result = await mediaService.read(request); if (!result.ok || result.value === undefined) throw new Error(result.error?.message ?? '视频读取失败。'); return result.value }
