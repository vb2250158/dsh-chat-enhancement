window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function isMediaToolName(toolName) { return ['read_image', 'show_image', 'show_video'].includes(toolName) }
    function CollapsibleToolCall({ block, sessionId, sessions, readVideo }) {
      ...
      return React.createElement('section', { 'data-dsh-chat-enhancement': 'tool-group', style: toolGroupStyle },
        ...
        expanded && React.createElement('div', { style: toolGroupBodyStyle }, isMediaToolName(toolName) ? React.createElement(MediaToolView, { block, sessionId, sessions, readVideo }) : React.createElement(GenericToolView, { toolName, block }), subCalls.map((child) => React.createElement(CollapsibleToolCall, { key: child.callId, block: child, sessionId, sessions, readVideo }))),
      )
    }
    function CollapsibleToolCallTree({ node, sessionId, sessions, readVideo }) { return React.createElement(CollapsibleToolCall, { block: node.data.root, sessionId, sessions, readVideo }) }
    ...
    async function apply(ctx) {
      ...
      ctx.slots.inject('conversation.chat.node', () => ctx.slots.register({ name: 'conversation.chat.node', key: 'tool-call', priority: -1, locale: 'conversation', inject: () => ({ sessions, readVideo }) }, CollapsibleToolCallTree))
